...
char scan[]= {0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80};
...
void display(char dat[][8],char go_speed)//显示动画的一帧，对光立方完全扫描一遍
{
    char speed;
    char y_line;
    char z_line;
    ...
    while(speed--)
    {
        for(y_line=0; y_line<8; y_line++)
        {
            P2=scan[y_line];//P2接的573片选

            for(z_line=0; z_line<8; z_line++)
            {
                P0=scan[z_line];//P0接的2803层选
                P1=dat[y_line][z_line];//P1接的数据输入口
                delay(5);//延时越长，灯越亮，太大貌似会失效？小于100吧，影响扫描速度
                P1=0x00;
                P0=0x00;//能消隐吧
            }
        }
